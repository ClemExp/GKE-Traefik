...
resource "null_resource" "traefik_deploy" {
  ...
  provisioner "local-exec" {
    command = "helm upgrade --install traefik ../helm/traefik-dashboard --values ../helm/traefik-dashboard/values.yaml --set varsubdomain='clemoregan.com' -n tlson --create-namespace"
